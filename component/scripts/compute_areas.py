...
def create_hist(result_dir, map_raster, alert):
    ...
    with rio.open(map_raster) as src:
        ...
        kwargs.update(
            {"crs": rio_crs, "transform": transform, "width": width, "height": height}
        )
...
def plot_loss(df, aoi_name):
    ...
    ax_y = Axis(
        label="tree cover loss surface (ha)", scale=y_sc, orientation="vertical"
    )
    bar = Bars(
        x=[i + 2000 for i in d_hist["code"]],
        y=d_hist["area"],
        scales={"x": x_sc, "y": y_sc},
    )
    title = f"Distribution of tree cover loss per year in {aoi_name}"
    fig = Figure(
        title=title, marks=[bar], axes=[ax_x, ax_y], padding_x=0.025, padding_y=0.025
    )
...
def area_table(df):
    ...
    items = [
        {"class": row["class"], "area": f"{int(row.area)}"}
...
def compute_ee_map(aoi_model, model):
    ...
    bands = {
        ...
        "B": clip_dataset.select("lossyear").unmask(
            0
        ),  # be carefull the 0 values are now masked
